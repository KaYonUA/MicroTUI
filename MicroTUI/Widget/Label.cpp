	...
	namespace GUI
	{
		void Label::_Render_func(ScreenBuffer *buffer, COORD _w_Coord)
		{
			...
			if(_f_isML == false)
				...
				_p_str->c_str() :
				widgettitle.c_str(), Color::_Transparent);
			else
			{
				buffer->Rectangle(_g_widgCord.X, _g_widgCord.Y,WidgetSize.width,WidgetSize.height,Color::Pixel(' ' ,L_colr));
				...
				for(int _t_len = 0; temp.size() >= _t_len; _t_len++)
				{
					if(temp[_t_len] == ' '|| temp[_t_len] == '\n'|| temp[_t_len] == '\0')
					{
						...
						{
							buffer->Lable(_g_widgCord.X + next_wp,_g_widgCord.Y+w_line,temp.substr(0,_t_len).c_str(),Color::_Transparent);
							avalib_sp -= wrd_leng;
						}
						else
						{
